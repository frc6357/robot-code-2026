...
public class FuelDetection extends SubsystemBase implements PathplannerSubsystem {
    ...
    public void addPathPlannerCommands() {
        ...
        PathPlannerCommands.addCommand("FuelHunt Full", FuelHuntCommand.create());

        // Simple collect pause — can be inserted in any auto sequence
        PathPlannerCommands.addCommand("Collect Fuel", Commands.waitSeconds(1.5).withName("CollectFuelWait"));

        // Scored return trench decision — used by the pre-planned FuelHunt.auto
        PathPlannerCommands.addCommand("FuelHunt Choose Return Trench", createScoredReturnCommand());
    }
    ...
    private static Command createScoredReturnCommand() {
        ...
        return Commands.defer(() -> {
            ...
            return Pathfinder.PathfindThenFollowPathCommand(chosenPath, kFuelHuntConstraints);
        }, 
